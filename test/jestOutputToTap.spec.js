const jestOutputToTap = require('../src/jestOutputToTap');
const {
  failingTestResult,
  severalTestResults,
  singleTestResult,
...
describe('mol-fe-jest-tap-results-processor', () => {
  it('must output the plan', () => {
    let tapResult = jestOutputToTap(singleTestResult);

    expect(tapResult).toEqual(string.startsWith('1..1'));

    tapResult = jestOutputToTap(severalTestResults);

    expect(tapResult).toEqual(string.startsWith('1..7'));
  });

  it('must properly output success test lines', () => {
    const tapResult = jestOutputToTap(severalTestResults);
  ...
  it('must properly output error test lines', () => {
    const tapResult = jestOutputToTap(failingTestResult);
    const output = processTestOutput(tapResult);
  ...
  it('must add the skiped directive to skiped test lines', () => {
    const tapResult = jestOutputToTap(skippedTestResult);
    const output = processTestOutput(tapResult);
